 ...
 * 26. April 2020: added a hh:mm based alarm
 * 
 * when:
 * 25. April 2020
 * 
...
// THis is the value of menue1
 //int i_val1 = 3;
 int i_val[] = {0, 3, 2, 735, 0, 0};
 const char* const c_menue[] = {"Month&Year", "Move every", "UTC+", "Al1Time", "Al1Status", "Menue5"};
   ...
   if ((i_posRotary1 != i_newPosRotary1) || b_displayMode || b_displayUpdate)
   {
      ...
      if (i_newPosRotary1 < ROTMIN)
      {
         ...
         switch (i_menue)
         {
            ...
            case 3: // Menue #3 - Al1Time
               Serial.println(c_menue[i_menue]);
               if (b_displayMode == 0){sprintf(c_num16, "%s: %02d:%02d", c_menue[i_menue], i_val[i_menue]/60, i_val[i_menue]%60 );}
               else{changeTime();}
               break; 
      
            case 4: // Menue #4 - Al1Status
               Serial.println(c_menue[i_menue]);
               if (b_displayMode == 0){sprintf(c_num16, "%s: %1d", c_menue[i_menue], i_val[i_menue]);}
               else{changeSingleInt();}
               break; 
...
///////////////////////////////////////////////////////////////
void changeSingleInt() {
   // used to change the default value
   // that is stored in i_val[i_menue]
   // 
   ...
   Serial.println("leaving changeSingleInt");
...
} // end of changeSingleInt

///////////////////////////////////////////////////////////////
void changeTime() {
   // Same as changeSingleInt(), but here the value is displayed as a time 
   // Divide minutes by 60 gives hours; remaining minutes by using modulo.
   //
   int i_backupRotary = enc_Rotary1.read();
   int i_subRotary = 0;
   int i_newSubRotary = 0;
   ...
   sprintf(c_num16, "*%s: %02d:%02d", c_menue[i_menue], i_val[i_menue]/60, i_val[i_menue]%60 );
   ...
   do {
      ...
      if (i_newSubRotary != i_subRotary){
         i_backlight = BACKLIGHT; 
         lcd.backlight();
         sprintf(c_num16, "*%s: %02d:%02d", c_menue[i_menue], i_newSubRotary/60, i_newSubRotary%60 );
      } // end of if
   } while (digitalRead(PIN_D12));

   Serial.println("leaving changeTime");
